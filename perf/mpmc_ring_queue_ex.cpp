...
#include "threads/mpmc_ring_queue_ex.h"
...
void produce_consume(CppBenchmark::Context& context, const std::function<void()>& wait_strategy)
{
    ...
    CppCommon::MPMCRingQueueEx<T> queue(N);
    ...
    context.metrics().SetCustom("MPMCRingQueueEx.capacity", N);
    context.metrics().SetCustom("CRC", crc);
}

BENCHMARK("MPMCRingQueueEx-SpinWait-producers", settings)
{
    produce_consume<int, 1048576>(context, []{});
}

BENCHMARK("MPMCRingQueueEx-YieldWait-producers", settings)
{
